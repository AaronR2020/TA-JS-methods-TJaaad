...
*/
var arr=[0,2,3,4,55,63,77,33,11,22,33,44,55,66];
...
function changeArray(arr,cb){
  ...
  console.log(newArr,`change Array`);
 return newArr;
}
changeArray(arr,addFive);
/*
...
function second(num){
  function third(num){
    console.log(num+1,`funct. within funct.`);
    return num+1;
  }
  return third(num);
}
...
console.log(callMe(callMeMayBe))
...
*/
function isAdult(peop){
return peop.age>18?  true: false
}

for(i in people){
 console.log(isAdult(people[i]));
}
...
*/
function isMaleOb(ob){
  return ob.sex=='M'? "M": "F";
}
function isMale(grads,cb){
for(i in grads){
  console.log(cb(grads[i]),`is your gender`)
}
}
isMale(grades,isMaleOb);
/*
...
*/
function isFeMaleOb(ob){
  return ob.sex=='M'? "Male": "Female";
}
function isFeMale(grads,cb){
for(i in grads){
  console.log(cb(grads[i]),`is your gender`)
}
}
isMale(grades,isFeMaleOb);
/*
...
*/
function isGrade(ob){
  return ob.grade>12? "greater": "less";
}
function isMale(grads,cb){
for(i in grads){
  console.log(cb(grads[i]),)
}
}
isMale(grades,isGrade);
/*
...
*/
function checkB(obj){
return obj.grade>8&&obj.grade<12?"B":`not B`;

}//does calculation

function isGradeB(arr,cb){
for(i in arr){
  console.log(cb(arr[i]))
}
}

isGradeB(grades,checkB);
/*
...
*/
function checkC(obj){
  return obj.grade<=8?"C":`not C`;
  
  }//does calculation
  
  function isGradeC(arr,cb){//we pass array elements to a function
  for(i in arr){
    console.log(cb(arr[i]))
  }
  }
  
  isGradeC(grades,checkC);
/*
...
function checkadult(obj){
  return obj.age>18?`name:${obj.name}, age:${obj.age}`:"";
...
function checkMale(obj){
  return obj.sex==`M`?`name:${obj.name}, grade:${obj.grade}: sex:${obj.sex}`:"";
...
function checkFemale(obj){
  ...
  filterFemale(grades,checkFemale);



//get back to this on the 20th.
